...
const MSFoundationBolt = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            {products.map((product, i) => (
              <motion.div key={product.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: i * 0.05 }} className="group relative bg-card rounded-2xl border border-border overflow-hidden hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 transition-all duration-500">
                ...
                <div className="p-5">
                  ...
                  <button onClick={() => setEnquiryProduct(product.name)} className="w-full btn-pill-primary px-5 py-2.5 text-sm gap-2 justify-center">
      ...
      <AnimatePresence>
        {enquiryProduct && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setEnquiryProduct(null)}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ duration: 0.2 }} className="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-md p-6 relative" onClick={(e) => e.stopPropagation()}>
              <button onClick={() => setEnquiryProduct(null)} className="absolute top-4 right-4 p-1.5 rounded-full hover:bg-secondary transition-colors text-muted-foreground"><X className="w-4 h-4" /></button>
              <h3 className="font-display text-lg font-bold text-foreground mb-1">Get Best Quote</h3>
              <p className="text-sm text-muted-foreground mb-5">For: <span className="text-foreground font-medium">{enquiryProduct}</span></p>
