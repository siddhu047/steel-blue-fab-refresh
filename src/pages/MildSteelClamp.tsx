...
const product = {
  name: "Iron Pole Clamps",
  price: "₹160",
  unit: "Piece",
  minOrder: "50 Pieces",
  specs: ["Material: Iron", "Pole type: Steel Pole", "All pole sizes", "Corrosion resistant", "Pack type: Box", "Made in India"],
  usage: ["Earthing Strip", "Street Light", "Overhead Line", "Cable Support"],
  category: "MS Clamp",
  img: "https://5.imimg.com/data5/SELLER/Default/2023/8/337380115/IL/NN/YY/105340714/hole-clamp-250x250.jpg",
};

const MildSteelClamp = () => {
      ...
      <section className="py-12 sm:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-12">
            ...
            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground">Mild Steel Clamp</h1>
            <p className="text-muted-foreground mt-3 max-w-xl text-sm leading-relaxed">Iron pole clamps for earthing strips, street lights, overhead lines and cable support — available in all pole sizes, corrosion resistant, packed in box.</p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="group relative bg-card rounded-2xl border border-border overflow-hidden hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 transition-all duration-500 max-w-[480px]">
            ...
            <div className="p-5">
              ...
              <div className="mb-4">
                <p className="text-xs font-medium text-foreground mb-1.5">Used for:</p>
                <div className="flex flex-wrap gap-1.5">
                  {product.usage.map((use) => (
                    <span key={use} className="px-2 py-0.5 rounded-full bg-primary/10 text-[11px] text-primary font-medium">{use}</span>
                  ))}
                </div>
              </div>
